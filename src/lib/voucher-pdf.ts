...
export async function buildVoucherPdf(document: VoucherRecord) {
  ...
  page.drawText(`Payment Voucher: ${document.type || "Payment"}`, {
    x: 58,
    y: pageHeight - 162,
    size: 14,
    font: boldFont,
  ...
  // Bank / Mobile Details on the right (below Category)
  if (document.paymentMethod === "Bank Transfer" && (document.bankName || document.accountNumber)) {
    drawStackedField(
      page,
      { bold: boldFont, regular: regularFont },
      rightLabelX,
      pageHeight - 350,
      "Bank Details",
      `${document.bankName || "-"} / ${document.accountNumber || "-"}`,
      text,
      muted,
    );
    drawStackedField(
  ...
  } else if (document.paymentMethod === "Mobile Money" && (document.mobileNumber || document.mobileReference)) {
    drawStackedField(
      page,
      { bold: boldFont, regular: regularFont },
      rightLabelX,
      pageHeight - 350,
      "Mobile Details",
      `${document.mobileNumber || "-"} (${document.payerName || "-"})`,
      text,
      muted,
    );
    drawStackedField(
